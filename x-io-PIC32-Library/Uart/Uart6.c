 ...
 */
void Uart6Initialise(const UartSettings * const uartSettings) {
...
static inline __attribute__((always_inline)) void TXInterruptTasks() {
    ...
    while (U6STAbits.UTXBF == 0) { // repeat while transmit buffer not full
        ...
        U6TXREG = writeBuffer[writeBufferReadIndex++ & READ_WRITE_BUFFER_INDEX_BIT_MASK];
    }
