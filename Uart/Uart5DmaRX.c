...
void Uart5DmaRXInitialise(const UartSettings * const settings, const UartDmaReadConditions * const readConditions) {
    ...
    DCH0ECONbits.CHSIRQ = _UART5_RX_VECTOR;
 ...
 */
void Uart5DmaRXSetReadCallback(void (*read)(const void* const data, const size_t numberOfBytes)) {
    EVIC_SourceDisable(INT_SOURCE_DMA0);
    readCallback = read;
    EVIC_SourceEnable(INT_SOURCE_DMA0);
}
 ...
 */
size_t Uart5DmaRXGetWriteAvailable() {
 ...
 */
void Uart5DmaRXClearWriteBuffer() {
 ...
 */
bool Uart5DmaRXHasReceiveBufferOverrun() {
    if (U5STAbits.OERR == 1) {
        U5STAbits.OERR = 0;
        return true;
    }
 ...
 */
bool Uart5DmaRXIsTransmitionComplete() {
