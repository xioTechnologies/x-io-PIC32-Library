...
#define INTERRUPT_VECTOR _I2C_2_VECTOR
#else
#define INTERRUPT_VECTOR _I2C2_MASTER_VECTOR
#endif
...
void I2C2Disable() {
    ...
    // Disable interrupt
    SYS_INT_SourceDisable(INT_SOURCE_I2C_2_MASTER);
    SYS_INT_SourceStatusClear(INT_SOURCE_I2C_2_MASTER);
}
...
uint8_t I2C2Receive(const bool ack) {
    ...
    // ACK/NACK
    SYS_INT_SourceStatusClear(INT_SOURCE_I2C_2_MASTER);
    I2C2CONbits.ACKDT = (ack == true) ? 0 : 1;
    I2C2CONbits.ACKEN = 1;
    WaitForInterruptOrTimeout();
    return I2C2RCV;
}
...
static void WaitForInterruptOrTimeout() {
    ...
    while (true) {
        if (SYS_INT_SourceStatusGet(INT_SOURCE_I2C_2_MASTER) == true) {
            break;
        }
 ...
 */
void I2C2BeginMessage(I2CMessage * const i2cMessage) {

    // Do nothing if message in progress
    if (I2C2MessageInProgress() == true) {
        return;
    }
    ...
    SYS_INT_SourceStatusSet(INT_SOURCE_I2C_2_MASTER);
    SYS_INT_SourceEnable(INT_SOURCE_I2C_2_MASTER);
}
 ...
 */
bool I2C2MessageInProgress() {
    if (TimerGetTicks64() >= messageTimeout) {
        SYS_INT_SourceDisable(INT_SOURCE_I2C_2_MASTER);
    }
    return SYS_INT_SourceIsEnabled(INT_SOURCE_I2C_2_MASTER);
}

/**
 * @brief I2C interrupt.
 */
void __ISR(INTERRUPT_VECTOR) I2C2Interrupt() {
    SYS_INT_SourceStatusClear(INT_SOURCE_I2C_2_MASTER); // clear interrupt flag first because next event may complete before ISR returns
    ...
    switch (currentI2CMessage->event[index]) {
        ...
        case I2CMessageEventSend:
            I2C2TRN = currentI2CMessage->data[index];
            break;
        case I2CMessageEventReceive:
            I2C2CONbits.RCEN = 1;
            break;
        case I2CMessageEventAck:
            *currentI2CMessage->destination[index] = I2C2RCV;
            I2C2CONbits.ACKDT = 0;
            I2C2CONbits.ACKEN = 1;
            break;
