 ...
 */
void Uart2InterruptHandler() {

    // RX interrupt
    if (EVIC_SourceStatusGet(INT_SOURCE_UART2_RX) == true) {
        RXInterruptTasks();
    }
    ...
    if (EVIC_SourceStatusGet(INT_SOURCE_UART2_TX) == true) {
        TXInterruptTasks();
    }
}
...
static inline __attribute__((always_inline)) void RXInterruptTasks() {
    while (U2STAbits.URXDA == 1) { // repeat while data available in receive buffer
        if (CircularBufferGetWriteAvailable(&readBuffer) == 0) { // if read buffer full
            ...
            CircularBufferWriteByte(&readBuffer, U2RXREG);
        }
    }
    EVIC_SourceStatusClear(INT_SOURCE_UART2_RX);
}
...
static inline __attribute__((always_inline)) void TXInterruptTasks() {
    ...
    while (U2STAbits.UTXBF == 0) { // repeat while transmit buffer not full
        ...
        U2TXREG = CircularBufferReadByte(&writeBuffer);
    }
