...
const InputCapture inputCapture5 = {
    .initialise = InputCapture5Initialise,
    .deinitialise = InputCapture5Deinitialise,
    .bufferOverrun = InputCapture5BufferOverrun,
};
...
void InputCapture5Initialise(const InputCaptureSettings * const settings, void (*const captureEvent_) (const uint64_t ticks)) {
    ...
    switch (settings->edge) {
        ...
        case InputCaptureEdgeEvery:
            IC5CONbits.ICM = 0b110; // simple Capture Event mode - every edge, specified edge first and every edge thereafter
            break;
    }
    switch (settings->interrupt) {
        ...
        case InputCaptureInterruptSecond:
            IC5CONbits.ICI = 0b01; // interrupt on every second capture event
            break;
        case InputCaptureInterruptThird:
            IC5CONbits.ICI = 0b10; // interrupt on every third capture event
            break;
        case InputCaptureInterruptFourth:
            IC5CONbits.ICI = 0b11; // interrupt on every forth capture event
            break;
    }
    IC5CONbits.FEDGE = settings->firstEdge ? 1 : 0;
    IC5CONbits.C32 = 1;
...
void InputCapture5InterruptHandler(void) {
    ...
    while (IC5CONbits.ICBNE) {
        const uint32_t latency = (uint32_t) now - (uint32_t) IC5BUF;
