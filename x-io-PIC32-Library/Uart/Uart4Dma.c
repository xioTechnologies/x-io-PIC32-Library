...
void Uart4DmaInitialise(const UartSettings * const settings, const UartDmaReadConditions * const readConditions) {
    ...
    DCH3ECONbits.CHSIRQ = INT_VECTOR_UART4_RX;
    ...
    DCH3SSA = KVA_TO_PA(&U4RXREG); // source address
    ...
    DCH4ECONbits.CHSIRQ = INT_VECTOR_UART4_RX;
 ...
 */
void Uart4DmaSetReadCallback(void (*read)(const void* const data, const size_t numberOfBytes)) {
    SYS_INT_SourceDisable(INT_SOURCE_DMA_3);
    readCallback = read;
    SYS_INT_SourceEnable(INT_SOURCE_DMA_3);
}
...
bool Uart4DmaIsTransmitionComplete() {
    return (Uart4DmaIsWriteInProgress() == false) && (U4STAbits.TRMT == 1);
}
