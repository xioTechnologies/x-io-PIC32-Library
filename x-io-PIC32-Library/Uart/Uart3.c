...
void Uart3Initialise(const UartSettings * const uartSettings) {
    ...
    U3MODEbits.PDSEL = uartSettings->parityAndData;
    U3MODEbits.STSEL = uartSettings->stopBits;
    ...
    U3BRG = UartCalculateUxbrg(uartSettings->baudRate);
    ...
    SYS_INT_VectorPrioritySet(INT_VECTOR_UART3, INTERRUPT_PRIORITY);
#else
    SYS_INT_VectorPrioritySet(INT_VECTOR_UART3_RX, INTERRUPT_PRIORITY);
    SYS_INT_VectorPrioritySet(INT_VECTOR_UART3_TX, INTERRUPT_PRIORITY);
#endif
    SYS_INT_SourceEnable(INT_SOURCE_USART_3_RECEIVE); // enable RX interrupt only
}
...
size_t Uart3GetReadAvailable() {
    ...
    if (U3STAbits.URXDA == 1) {
        SYS_INT_SourceStatusSet(INT_SOURCE_USART_3_RECEIVE);
    }
 ...
 */
void __ISR(_UART_3_VECTOR) Uart3Interrupt() {

    // RX interrupt
    if (SYS_INT_SourceStatusGet(INT_SOURCE_USART_3_RECEIVE) == true) {
        RXInterruptTasks();
    }

    // TX interrupt
    if (SYS_INT_SourceIsEnabled(INT_SOURCE_USART_3_TRANSMIT) == false) {
        return; // return if TX interrupt disabled because TX interrupt flag will remain set while the transmit buffer is empty
    }
    if (SYS_INT_SourceStatusGet(INT_SOURCE_USART_3_TRANSMIT) == true) {
        TXInterruptTasks();
    }
}
 ...
 */
void __ISR(_UART3_RX_VECTOR) Uart3RxInterrupt() {
    RXInterruptTasks();
}
 ...
 */
void __ISR(_UART3_TX_VECTOR) Uart3TxInterrupt() {
    TXInterruptTasks();
}
...
static inline __attribute__((always_inline)) void TXInterruptTasks() {
    ...
    while (U3STAbits.UTXBF == 0) { // repeat while transmit buffer not full
        ...
        U3TXREG = writeBuffer[writeBufferReadIndex++ & READ_WRITE_BUFFER_INDEX_BIT_MASK];
    }
